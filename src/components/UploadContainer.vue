<template>
  <div :class="[
    showContainer == true
      ...
      : 'hidden sm:flex',
    maximized ? 'w-screen fixed left-0 top-0 justify-center items-center' : 'w-[560px] flex-col justify-between items-center',
    'upload-container flex',
  ]">
    <div
      :class="[maximized ? 'flex flex-col justify-evenly p-12 h-full' : 'sm:grid sm:grid-cols-2 sm:gap-5 gap-1', 'options-wrapper flex place-items-center']">
      <div class="paste-type group" @click="activePasteBox = 'text'">
        <svg class="group-hover:rotate-180 min-w-[70px] sm:min-w-[8vw]" viewBox="0 0 100 100" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <rect width="100" height="100" rx="30" :fill="activePasteBox == 'text'
            ? 'var(--active-color)'
            : 'var(--primary-color)'
            " />
        </svg>
      ...
      <div class="paste-type group" @click="activePasteBox = 'images'">
        <svg class="group-hover:rotate-145 min-w-[70px] sm:min-w-[8vw]" viewBox="0 0 210 204" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <div class="paste-type group" @click="activePasteBox = 'files'">
        <svg
          ...
          viewBox="0 0 225 202" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            ...
            :fill="activePasteBox == 'files'
              ? 'var(--active-color)'
              : 'var(--primary-color)'
              " />
        </svg>

        <p class="paste-type-text translate-y-[1px] group-hover:-translate-y-[5px]">
          Files
        </p>
      </div>

      <div class="paste-type group" @click="activePasteBox = 'code'">
        <svg viewBox="0 0 232 230" class="group-hover:rotate-90 min-w-[70px] sm:min-w-[8vw]" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
    ...
    <div class="absolute top-2 right-2 hidden sm:block">
      <button v-if="maximized" class="z-5 rounded-xl hover:rounded-[40px] bg-app-bg  w-10 h-10 shadow-sm"
        @click="maximized = false">
        <img class="w-4 icon m-auto" src="../assets/minimize-icon.svg" alt="minimize icon" />
      </button>
      <button v-else class="z-5 rounded-xl hover:rounded-[40px] bg-app-bg  w-10 h-10 shadow-sm" @click="maximized = true">
        <img class="w-3 icon m-auto" src="../assets/expand-icon.svg" alt="maximize icon" />
      </button>
    </div>
    <div
      :class="[maximized ? 'h-[90%] mr-12 p-5' : 'h-[80%] w-[90%] p-3', 'paste-input-wrapper relative bg-primary-color grow rounded-3xl']">
      <div class="flex flex-col items-center gap-4 justify-center w-full  h-full">
        <hr class="h-[8px] w-[40%] hidden bg-primary-light border-none rounded-full mb-2" />
        <text-uploader v-if="activePasteBox == 'text'" ref="textUploader" />
        <code-uploader v-if="activePasteBox == 'code'" ref="codeUploader" />
        <file-uploader v-if="activePasteBox == 'files'" ref="fileUploader" />
        <image-uploader v-if="activePasteBox == 'images'" ref="imageUploader" />
        <div class="flex gap-2">
          <button @click="upload"
            class="bg-btn-color px-10 py-2 rounded-[40px] font-medium hover:rounded-xl sm:hover:saturate-150">
            Upload
          </button>
          <button @click="emit"
            class="bg-btn-color px-10 py-2 rounded-[40px] font-medium hover:rounded-xl sm:hover:saturate-150">Emit</button>
        </div>
      </div>
    </div>
  </div>

  <button @click="$emit('toggleContainer')" :class="[
